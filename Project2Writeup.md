...
- First, given that an addition and moving a number from one processor to another both take a unit time. In each step, we are (1) adding two numbers and (2) moving a number to another processor. So the communication time equals the computation time.
- Second, if sending a number from processor p to p+-k takes time k, the communication time increases linearly with distance k, but the number of steps to complete the reduction is log in the size of n, since the number of numbers reduces by half in each step. So the execution time of the parallel algorithm is of the same order as the sequential time.|
| Exercise 2.4 |
- The case of summing 8 elements with 4 processors.
  ...
  - Step 3: One processor sums the two results in Step 2. (1 communication)
  - Comparing to Figure 2.3 of the textbook, this is the same as saying the edges connected to the top layer of the graph do not correspond to actual communications.
- The case of summing 16 elements with 4 processors.
  - Initial Condition: Each processor has four elements.
  - Step 1: Each processor sums two pairs of elements into two numbers. (no communication)
