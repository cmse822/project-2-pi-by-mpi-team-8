...
| Exercise 2.22 | Reference page 115 | A pseudocode of non-blocking approach will be as thus (for each step): Define next = (myRank + 1) % numProcessor; prev = (myRank + 1 - numProcessor) % numProcessor; MPI_Irecv(&x_ip from prev, recv_request); MPI_Isend(&y_i to next, send_request); MPI_Wait(recv_request); y_i = y_i + x_ip; MPI_Wait(send_request); A disadvantage of this non-bloacking communication would be the increased complesxity to take care of each 'wait' for the corresponding receive/send. Wrong placements of these commands lead to incorrect variables being computed, and thus produce wrong results. Also, an advantage of non-blocking would be to communicate and compute at the same time, so if there's not much computation and the code needs much more time to wait for communication but finishes the computation step in a short time, or simply when the wait command is placed too early. This may result an inefficient code performance.
